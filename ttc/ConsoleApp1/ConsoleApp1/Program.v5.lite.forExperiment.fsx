...
//// 定义一个帮助函数来格式化 Tensor 的内容
//let formatTensorContent (tensor: torch.Tensor) =
//    if tensor.numel() <= 10L then
//        tensor.ToString()
//    else
//        tensor.slice(0L, 0L, 10L,1L).ToString() + " ..."

//let formatTensorContent (tensor: torch.Tensor) =
//    if tensor.numel() < 10L then // 如果元素少于10个，显示所有内容
//        tensor.ToString()
//    else // 否则显示前10个元元素并附加省略号
//        tensor.slice(0L, 0L, 10L,1L).ToString() + " ..."
...
//    let content = formatTensorContent tensor
//    sprintf "Tensor (%A, shape=%A): %s" dtype shape content
//)

let formatTensorContent (tensor: torch.Tensor) =
    // 控制输出张量元素的数量
    let maxElementsToShow = 50L
    let numel = tensor.numel()
    let elementsToShow = min numel maxElementsToShow

    // 获取张量的部分或全部数据
    let data = tensor.slice(0L, 0L, elementsToShow - 1L, 1L)
    
    // 将张量转换为 .NET 数组，并生成字符串表示
    let values = data.data<single>().ToArray() // 这里假设张量是 float 类型
    let content = 
        values
        |> Array.map string
        |> String.concat ", "
    
    sprintf "[%s%s]" content (if numel > maxElementsToShow then ", ..." else "")

// 添加自定义打印器
fsi.AddPrinter(fun (tensor: torch.Tensor) ->
    let shape = tensor.shape
    let dtype = tensor.dtype
    sprintf "Tensor (%A, shape=%A): %s" dtype shape (formatTensorContent tensor)
)
...
(b2f trainSet[0]).SegmentIds|>Seq.tryFindIndex (fun v -> v <> 0)

(b2f trainSet[0]).Tokens[121]
(b2f trainSet[0]).Tokens[122]

trainSet |> Seq.find (fun s -> (b2f s).SegmentIds |> Seq.distinct |> Seq.length > 2)
...
linearLayer.``to``(device) |> ignore // 将层移动到适当的设备

// 创建一个随机输入张量，形状为 [3, 10]
let inputTensor = torch.rand([|3L; 10L|], device = device)

inputTensor.data<single>()|> Seq.toArray

// 将输入张量通过全连接层
let outputTensor = linearLayer.forward(inputTensor)

printfn "Output Tensor: %A" outputTensor
printfn "Shape of Output Tensor: %A" outputTensor.shape
