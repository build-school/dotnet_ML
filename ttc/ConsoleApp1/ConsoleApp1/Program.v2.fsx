...
let toXY dev (batchId:string * int)  =
    ...
    let tokenIds = torch.tensor(d_tkns,     dtype=torch.int, device=dev).view(-1L,MAX_POS_EMB)        
    let sepIds   = torch.tensor(d_tkn_typs, dtype=torch.int, device=dev).view(-1L,MAX_POS_EMB)
    let Y = torch.tensor(toLabel batchId, dtype=torch.int64, device=dev).view(-1L)
    (tokenIds,sepIds),Y


testBert.eval()
let (_tkns,_seps),_ = toXY device ("train", 0) 
//_tkns.shape
...
let train () =
    ...
    while e < EPOCHS do
        ...
        let toxyed = 
            ...
            |> Seq.toArray
        let batch = toXY device trainSetIDs[0]
        let i = 0

        //toxyed[0]

        printfn "trainBatches length: %d" (toxyed.Length)
...
let save_model (model: torch.nn.Module) (fileDir: string) =
    ...
    stateDict
    |> Seq.iter (fun kvp ->
        torch.save(kvp.Value,Path.Join(fileDir, kvp.Key))        
    )



save_model _model @"C:\anibal\ttc\MyTrainedModel"
