...
type MinDataLoader(dataset : MinDataset, batch_size, ?shuffle, ?num_worker, ?drop_last) =
    inherit Modules.DataLoader<Tensor * Tensor, Tensor * Tensor>(dataset, batch_size, MinDataLoader.Collate, ?shuffle=shuffle, ?num_worker=num_worker, ?drop_last=drop_last)
