...
type CausalSelfAttention(config) as self =
    ...
    override _.forward(x) =
        ...
        let att =
            ...
            let (struct (mask, _)) = bias //[Colon, Colon, Slice(stop=T), Slice(stop=T)]
            att.masked_fill(torch.eq(mask[Colon, Colon, Slice(stop=T), Slice(stop=T)], 0), Double.NegativeInfinity)
